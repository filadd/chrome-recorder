...
- The backend — **`chrome-recorder-consumer-api`** (FastAPI + Celery, separate repo) — owns the three flows the extension talks to (Upload) or that run behind it (Processing, Delivery):
  ...
  - **Processing** (Celery Beat): list the recordings bucket (skip recordings already claimed in Notion) → claim each by creating a Notion Transcription in `processing` (stamped with the recording key) → diarized transcription (`transcript-api`, Deepgram) → write the transcript into the Notion Transcription page body (a Speaker legend + the spoken lines) → promote to `pending` → delete the audio. On failure the row is set to `failed` and the audio kept; the claim prevents a long run (one outlasting the Beat interval) being reprocessed into duplicate rows.
  - **Delivery** (Celery Beat): for transcriptions a reviewer marked `speakers_assigned`, rebuild the named transcript → rewrite the Transcription page body with the assigned names (so it reads as the finished conversation) → `ai-conversations-api` context generation → upsert the Notion Context page body → mark `delivered`.
...
- **The server owns the parts ledger** (`file-uploads-api`'s Redis). The client never holds a ledger: it PUTs each part to the presigned URL it holds, reads the `ETag`, and reports it to the consumer API's `POST /uploads/part/`, which records it and returns the **next** part's URL. Part numbers are consecutive from 1; the bucket CORS must expose the `ETag` header (else `res.headers.get("ETag")` is null). The part-1 URL is minted at create but only PUT once the first 5 MiB buffers (~5+ min), so the consumer API sends a per-upload `presigned_url_expiration` (`UPLOAD_PRESIGNED_URL_EXPIRATION`, default 3600 s) — file-uploads' 300 s default would expire it.
...
- **The recordings bucket is transient** (managed outside this repo). `filadd-chrome-recorder-prod` (final destination, region **`sa-east-1`** — the consumer API's `AWS_S3_REGION` must match or presigned GETs 400) expires objects after 7 days; `file-uploads-api-prod` (file-uploads staging) has an `AbortIncompleteMultipartUpload` rule.
- **Notion (Processing/Delivery) gotchas — handled in `chrome-recorder-consumer-api`, but relevant to the data model.** Resolving `Recorded by` (person) from an email needs a Notion **integration token with the user-read capability** (a Personal Access Token gets `403` on `GET /v1/users`, leaving it blank). The **diarized transcript lives in the Transcription page body** (a `## Speakers` legend the reviewer fills in — `Speaker N → name` — plus `Speaker N [mm:ss]: text` lines), not in separate Segment/Speaker rows; Delivery reads the body back and maps the legend onto the lines. The **living Context is the page body** too (markdown blocks), not a property. **Database templates can't be applied via the API**, and an invalid **Pitch relation is silently dropped** (validate `pitchId` is a real Pitches page). See spec.md §7.
