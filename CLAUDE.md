...
Generic Chrome MV3 extension that records Google Meet calls (tab audio + microphone) and streams the recording to S3 via multipart upload while the call is in progress. S3 is **transient**: an event-driven AWS pipeline (S3 `ObjectCreated` → Ingest Lambda → Deepgram async callback → Callback Lambda) transcribes each object, routes the output per *profile* (`project` → transcript + living context page under a Notion pitch), and deletes the audio. Successor to `lowcode-orientation-transcriptor-extension`. Only the `project` profile ships today; the machinery stays generic so others can be re-added.
...
- `api/` — the upload API **and** the processing pipeline (Node + Hono + AWS SDK v3), one SAM stack. `app.ts`/`lambda.ts` are the multipart signer (Function URL); `ingest.ts` (S3 event → Deepgram async submit) and `callback.ts` (Deepgram callback → format/LLM/Notion/delete, own Function URL) are the pipeline, with logic in `src/pipeline/{secrets,deepgram,transcript,llm,notion,routing}.ts`. `index.ts` serves the signer locally. SAM (`template.yaml`) provisions all functions plus the bucket (created + owned by the stack, `DeletionPolicy: Retain`). Own `package.json`.
...
- **Object metadata is the pipeline contract** (`pitch_id`, `participants`, `meet_slug`, `recorded_by`, `started_at`): set once at `CreateMultipartUpload`, immutable, 2 KB total, ASCII-only values — the API maps/sanitizes them (`api/src/keys.ts`). Mutable per-object state (`status` tag) goes in object *tags*, not metadata.
- **Pipeline idempotency**: the Callback Lambda deletes the object on success, so a duplicate Deepgram callback (or S3 event) for a deleted key no-ops; a `status` tag (`submitted`/`processing`) guards in-flight work. Deepgram re-POSTs on 5xx (~10×) is the only retry — there is no poller.
